...
# 2. Create Frankfurt prospective customers
$PSQL -f $DIR/create_colt_prospects_frankfurt.sql
cat /$DIR/local_data/colt_prospects_frankfurt.csv | ${PSQL} -a -c "COPY source_colt.prospects_frankfurt\
    (company_name,company_label,address,postcode,city,sic_4,sic_2,sic_industry_german,sic_industry_english,solon_av_industry,employees,employees_bracket,lat,lon) \
    FROM STDIN DELIMITER ',' CSV HEADER encoding 'windows-1251';"


# Create and load the CO data from Paris and Frankfurt. Paris and Frankfurt data came in two separate formats, so we made 2 tables.
