...
if [ ${fiber_state_array_len} == 1 ]; then
	...
	if [ ! -f $GISROOT/geotel_fiber_${FIBER_STATE_ARRAY[0]}.zip ]; then
...
# If there are two or more states, download the first file, create the table, then loop through the rest and append
	...
	do
		rm -f ${TMPDIR}/*.*
	        if [ ! -f $GISROOT/geotel_fiber_${FIBER_STATE_ARRAY[i]}.zip ]; then
...
if [ ${wirecenter_state_array_len} == 1 ]; then
	rm -f ${TMPDIR}/*.*
        if [ ! -f $GISROOT/geotel_wirecenters_${WIRECENTER_STATE_ARRAY[0]}.zip ]; then
...
# If there are two or more states, download the first file, create the table, then loop through the rest and append
	...
	do
		rm -f ${TMPDIR}/*.*
	        if [ ! -f $GISROOT/geotel_wirecenters_${WIRECENTER_STATE_ARRAY[i]}.zip ]; then
