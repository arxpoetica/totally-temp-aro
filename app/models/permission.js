...
Permission.grant_access = function(plan_id, user_id, rol, callback) {
  txain(function(callback) {
    var sql = 'DELETE FROM auth.permissions WHERE route_id=$1 AND user_id=$2';
    database.execute(sql, [plan_id, user_id], callback);
  })
  .then(function(callback) {
    var sql = 'INSERT INTO auth.permissions (route_id, user_id, rol) VALUES ($1, $2, $3)';
...
Permission.find_permission = function(plan_id, user_id, callback) {
  var sql = 'SELECT rol FROM auth.permissions WHERE route_id=$1 AND user_id=$2';
