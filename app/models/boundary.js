...
Boundary.delete_boundary = function(boundary_id, callback) {
  var sql = 'DELETE FROM custom.boundaries WHERE id=$1';
  database.execute(sql, [boundary_id], callback);
};

Boundary.edit_boundary = function(data, callback) {
  var sql = 'UPDATE custom.boundaries SET name=$1, geom=ST_GeomFromGeoJSON($2) WHERE id=$3';
  var params = [
    data.name,
    data.geom,
    data.id,
