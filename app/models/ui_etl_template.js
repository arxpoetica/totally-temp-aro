...
module.exports = class UiEtlTemplate {
  static getEtlTemplateNamesByType (dataType) {
    ...
    return database.query(clientSql, [clientName])
      .then( results => {
        ...
        const templateSql = 'SELECT id, name FROM ui.etl_template WHERE client_id=$1 AND data_type=$2'
  ...
  static getEtlTemplateFileText (templateId) {
    const templateSql = 'SELECT name, template FROM ui.etl_template WHERE id=$1'
