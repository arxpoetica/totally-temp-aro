...
app.service('optimization', ($rootScope, $http, $q) => {
  ...
  optimization.optimize = (plan, changes, success, error) => {
    ...
    function checkNumberOfAreas () {
      if (changes.algorithm === 'ABCD' && changes.geographies.length >= 15) {
        var timing = changes.geographies.length
        swal({
          title: '',
          text: `You are running a ABCD analysis covering ${changes.geographies.length} service areas. While optimization is running, you will be sent back to the homescreen. Expected timing of optimization is ${timing} minutes.`,
          type: 'info',
