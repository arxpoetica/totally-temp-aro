class Sockets {
  constructor (app) {
    ...
    Object.keys(this.sockets).forEach(socketKey => {
      console.log(`Setting up room join/leave for socket namespace ${socketKey}`)
      const thisSocket = this.sockets[socketKey]
      thisSocket.on('connection', socket => {
        console.log(`Client with id ${socket.client.id} connected to socket with namespace ${socketKey}`)
        socket.on('SOCKET_JOIN_ROOM', roomId => {
          console.log(`Client with id ${socket.client.id} joined room ${roomId} on socket with namespace ${socketKey}`)
          socket.join(`${roomId}`)
        })
        socket.on('SOCKET_LEAVE_ROOM', roomId => {
          console.log(`Client with id ${socket.client.id} left room ${roomId} on socket with namespace ${socketKey}`)
          socket.leave(`${roomId}`)
        })
      })
    })
  ...
  emitToAll (payload) {
    // Here in previous, message is emitted to 'this.sockets.client', it should be 'this.sockets.broadcast' for broadcast.
