...
class Sockets {
  constructor (app) {
    ...
    Object.keys(this.sockets).forEach(socketKey => {
      helpers.logger.info(`Setting up room join/leave for socket namespace ${socketKey}`)
      const thisSocket = this.sockets[socketKey]
      thisSocket.on('connection', socket => {
        helpers.logger.info(`Client with id ${socket.client.id} connected to socket with namespace ${socketKey}`)
        socket.on('SOCKET_JOIN_ROOM', roomId => {
          helpers.logger.info(`Client with id ${socket.client.id} joined room ${roomId} on socket with namespace ${socketKey}`)
          socket.join(`${roomId}`)
        })
        socket.on('SOCKET_LEAVE_ROOM', roomId => {
          helpers.logger.info(`Client with id ${socket.client.id} left room ${roomId} on socket with namespace ${socketKey}`)
          socket.leave(`${roomId}`)
        })
      })
    })
  ...
  emitToClient (clientId, payload) {
    ...
    helpers.logger.info(`SOCKET EMIT client:${clientId}, payload: ${this.formatPayloadForLogging(payload)}`)
  }
  ...
  emitToUser (userId, payload) {
    ...
    helpers.logger.info(`SOCKET EMIT user:${userId}, payload: ${this.formatPayloadForLogging(payload)}`)
  }
  ...
  emitToPlan (planId, payload) {
    ...
    helpers.logger.info(`SOCKET EMIT plan:${planId}, payload: ${this.formatPayloadForLogging(payload)}`)
  }
  ...
  emitToLibrary (libraryId, payload) {
    ...
    helpers.logger.info(`SOCKET EMIT library:${libraryId}, payload: ${this.formatPayloadForLogging(payload)}`)
  }
  ...
  emitToAll (payload) {
    ...
    helpers.logger.info(`SOCKET EMIT broadcast, payload: ${this.formatPayloadForLogging(payload)}`)
  }
