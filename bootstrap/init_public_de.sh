...
if [ -z "$STATE_CODES" ]; then
  # Germany administrative areas
  export STATE_CODES='bw,by,be,br,hb,hh,he,mv,ni,nw,rp,sl,sn,st,sh,th'
fi
...
make reset_de_stage_public
make load_de_stage_public

make reset_view
make load_view

make reset_de_public
make load_de_public
