...
//			System.out.println(logString);
	...
	private Map<Long, LocationDemand> queryLocationDemand(long planId, int year) {
	...
	private Map<Long, RoadLocation> queryRoadLocations(long planId) {
		...
		planRepository.queryAllLocationsByPlanId(planId).stream()
				.map(OrdinalEntityFactory.FACTORY::createOrdinalEntity).forEach(result -> {
	...
	private Collection<NetworkAssignment> queryFiberSources(long planId) {
		...
		return toValidAssignments(planRepository.querySourceLocations(planId).stream()
	...
	private Collection<RoadEdge> queryRoadEdges(long planId) {
		return planRepository.queryRoadEdgesbyPlanId(planId).stream()
				.map(OrdinalEntityFactory.FACTORY::createOrdinalEntity).map(result -> {
					try {
