...
${PSQL} -c "DROP SCHEMA IF EXISTS sti CASCADE;"
${PSQL} -c "CREATE SCHEMA sti;"


# Use lower case state names. FIPS codes unnecessary here as well.
echo 'Creating empty households table so that aro.census_blocks can be generated correctly - this needs to be fixed at some point'
${PSQL} -a -f $DIR/sql/create_sti_households.sql
