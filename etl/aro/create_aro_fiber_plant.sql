...
INSERT INTO aro.fiber_plant (gid, carrier_name, cbsa, state, plant_type, zipcode, geog, geom)
	SELECT
		...
		type AS plant_type,
		zip AS zipcode,
		Geography(ST_GeometryN(ST_Transform(ST_Force_2D(the_geom), 4326),1)) as geog, -- Use ST_Force_2D because the source shapefiles have geometry type MultiLineStringZ...
		ST_GeometryN(ST_Transform(ST_Force_2D(the_geom), 4326),1) AS geom -- Use ST_Force_2D because the source shapefiles have geometry type MultiLineStringZ...
